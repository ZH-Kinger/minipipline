...
def render_episode(
    ...
    """Render one episode's keypoint-overlay (and optional depth) MP4."""
    ffb = check_ffmpeg()
    ...
    out_w = w * 2 if with_depth and depth_rgb is not None else w
    ...
    for t in range(T):
        ...
        composed = np.asarray(img)
        if with_depth and depth_rgb is not None and t < len(depth_rgb):
            composed = np.concatenate([composed, _depth_to_rgb(depth_rgb[t])], axis=1)
...
def visualize_dataset(
    ...
    with_depth: bool = False, out_dir: str | Path | None = None,
) -> list[Path]:
    """Render overlays for the given episodes (default: first 3)."""
    ...
    written: list[Path] = []
    for ep in episodes:
        written.append(render_episode(dataset_root, ep, out, with_depth=with_depth, fps=fps))
    return written
